...
def test_load_photoscans(example_database:Database, tmp_path:Path):
    ...
    photoscan = Photoscan()
    photoscan.from_file(photoscan_info["model_abspath"],photoscan_info["texture_abspath"])
    ...
    with pytest.raises(ValueError, match="not supported by reader"):
        photoscan.from_file(photoscan_info["model_abspath"], bogus_file)
    with pytest.raises(ValueError, match="not supported by reader"):
        photoscan.from_file(bogus_file, photoscan_info["texture_abspath"])
