...
from openlifu.cloud.api.dto import CreateObjectRequestDto, SystemDto
...
class SystemsApi:
    ...
    def get_config(self, system_id: int) -> bytes:
        return self._request.get_bytes(f"/systems/{system_id}/file/{CONFIG_FILE}")

    def upload_config(self, system_id: int, file: bytes, modification_date: datetime):
        url = f"/systems/{system_id}/file/{CONFIG_FILE}?modification_date={to_isoformat(modification_date)}"
