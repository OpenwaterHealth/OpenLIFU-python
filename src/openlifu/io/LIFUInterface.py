...
class LIFUInterface:
    ...
    def __init__(self,
                 ...
                 voltage_table: Optional[str] = None,
                 sequence_time: Optional[str] = None) -> None:
        """
    ...
    def check_solution(self, solution: Solution | Dict) -> None:
        ...
        self.voltage_table = self._resolve_voltage_chart_evt_version(self.voltage_table)
        self.sequence_time = self._resolve_max_sequence_time_set(self.sequence_time)
        sequence_duty_cycle = self.get_sequence_duty_cycle(solution)
        duty_cycles_limits = np.array(REF_MAX_DUTY_CYCLES)
